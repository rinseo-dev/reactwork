...
*/
    ...
    let [num,setNum] = useState(''); // 아래 input onChange

    /*useState(true)를 만들어서 2초 후에 false로 바꾸면 됨..?*/
    //let [alert,setAlert] = useState(true);

  /*  useEffect(()=> {
        // setAlert(true); // 이걸 넣어주면 count가 변경될 때 실행..?
        setTimeout(() => {
            setAlert(false)
        }, 2000);
    },[count]);*/
...
*/
    useEffect(()=>{
        if(isNaN(num)==true){// isNaN()으로 괄호안에 있는 값이 문자인지 확인
            alert('숫자만 넣으셈');
        }
    },[num]) // num값이 바뀌었을 때에만 재렌더링
